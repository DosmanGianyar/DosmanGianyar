...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 space-y-3 sm:space-y-0 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
    <form method="GET" action="{{ route('guru.attendance.rekap') }}" class="flex flex-col sm:flex-row gap-3 sm:items-end flex-1">
        <div class="flex-1">
    ...
    <div class="flex items-center gap-2 border-t sm:border-t-0 pt-3 sm:pt-0 border-gray-100 shrink-0">
        <button type="button" @click="showPreviewModal = true"
            class="inline-flex items-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl shadow-xs transition-colors">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
            Preview Cetak Laporan (PDF)
        </button>
        <a href="{{ route('guru.export.attendance.grid-pdf', ['month' => sprintf('%04d-%02d', $year, $month), 'class_id' => $selectedClassId]) }}"
            target="_blank"
            class="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-xl shadow-xs transition-colors">
...
{{-- ─── Modal Web Preview Laporan PDF ──────────────────────────────────── --}}
<div x-show="showPreviewModal" x-cloak
    @keydown.escape.window="showPreviewModal = false"
    class="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/70 backdrop-blur-xs">
    <div class="bg-white rounded-2xl w-full max-w-6xl h-[92vh] flex flex-col shadow-2xl border border-gray-100 overflow-hidden" @click.stop>
        {{-- Header Modal --}}
        <div class="px-5 py-3.5 bg-gray-900 text-white flex items-center justify-between shrink-0">
            <div class="flex items-center gap-3">
                <span class="p-1.5 bg-emerald-500/20 text-emerald-400 rounded-lg">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                </span>
                <div>
                    <h3 class="text-sm font-bold text-white">Pratinjau Dokumen Cetak Rekapitulasi Absensi Siswa Bulanan</h3>
                    <p class="text-[11px] text-gray-400">SMA Negeri 1 Gianyar · Dokumen Resmi PDF</p>
                </div>
            </div>
        ...
        {{-- Iframe Preview --}}
        <div class="flex-1 bg-gray-100 p-2 sm:p-4 overflow-hidden">
            <iframe src="{{ route('guru.export.attendance.grid-preview', ['month' => sprintf('%04d-%02d', $year, $month), 'class_id' => $selectedClassId]) }}"
                class="w-full h-full bg-white rounded-xl border border-gray-300 shadow-md"></iframe>
        </div>
    </div>
</div>
