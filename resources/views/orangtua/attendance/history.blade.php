...
<div class="bg-linear-to-br from-blue-600 to-indigo-700 rounded-2xl p-4 mb-4 text-white">
    <div class="flex items-center justify-between mb-3">
        ...
        <div class="text-center">
            ...
            <p class="text-blue-200 text-xs mt-0.5">{{ $student->name }}</p>
        </div>
        @if($canNext)
        ...
        <div class="flex-1 min-w-0">
            <div class="flex items-center justify-between gap-2">
                <p class="text-sm font-semibold text-gray-800">
                    {{ \Illuminate\Support\Carbon::parse($rec['date'])->isoFormat('dddd, D MMM Y') }}
                </p>
            ...
            <div class="flex items-center gap-3 mt-0.5 text-xs text-gray-400">
                ...
                <span>Masuk {{ \Illuminate\Support\Carbon::parse($rec['check_in_time'])->format('H:i') }}</span>
                @endif
                @if($rec['check_out_time'])
                <span>· Pulang {{ \Illuminate\Support\Carbon::parse($rec['check_out_time'])->format('H:i') }}</span>
                @endif
                @if($rec['is_fake_gps'])
