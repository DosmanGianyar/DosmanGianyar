...
<li
    ...
    @if ($isToggle)
        x-data="{ open: {{ ($active || $activeChildItems) ? 'true' : 'false' }} }"
    @endif
>
    @if ($isToggle)
        {{-- Parent toggle button (no URL, has children) --}}
        <button
            type="button"
            x-on:click="open = ! open"
            class="fi-sidebar-item-btn fi-sidebar-item-toggle"
        >
            @if (filled($icon))
                {{ \Filament\Support\generate_icon_html($icon, attributes: (new \Illuminate\View\ComponentAttributeBag)->class(['fi-sidebar-item-icon']), size: \Filament\Support\Enums\IconSize::Large) }}
            @endif

            <span class="fi-sidebar-item-label">{{ $slot }}</span>

            <svg
                :style="`width:1rem;height:1rem;flex-shrink:0;margin-left:auto;opacity:0.5;transition:transform 0.2s ease;transform:${open ? 'rotate(180deg)' : 'rotate(0deg)'}`"
