...
class _GuruEarlyCheckoutScreenState extends State<GuruEarlyCheckoutScreen> {
  ...
  Future<void> _load({bool reset = false}) async {
    ...
    try {
      final result = await GuruService.getEarlyCheckouts(status: _status, classId: _selectedClassId, page: _page);
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Persetujuan Pulang Lebih Awal')),
      body: Column(
        children: [
          _buildClassSelector(),
          _buildTabBar(),
  ...
  Widget _buildBody() {
    ...
    if (_items.isEmpty) {
      return const Center(
        child: Text('Tidak ada pengajuan pulang lebih awal', style: TextStyle(color: AppColors.gray400)),
      );
    }
          ...
          Row(
            children: [
              ...
              Container(
                ...
                child: Text(
                  'Jam ${item.requestedTime}',
                  style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: AppColors.orange700),
                ),
              ),
            ],
          ),
