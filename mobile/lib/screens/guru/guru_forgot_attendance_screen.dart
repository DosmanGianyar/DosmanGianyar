...
class GuruForgotAttendanceScreen extends StatefulWidget {
  const GuruForgotAttendanceScreen({super.key});

  @override
  State<GuruForgotAttendanceScreen> createState() => _GuruForgotAttendanceScreenState();
}
...
class _GuruForgotAttendanceScreenState extends State<GuruForgotAttendanceScreen> {
  ...
  Future<void> _approve(GuruForgotAttendance item) async {
    final confirm = await showApproveDialog(context, title: 'Setujui Lupa Absen?');
    if (confirm != true || !mounted) return;
    try {
      await GuruService.approveForgotAttendance(item.id);
      _load(reset: true);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Disetujui. Presensi dicatat Hadir'), backgroundColor: AppColors.green600),
        );
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _reject(GuruForgotAttendance item) async {
    final note = await showRejectDialog(context, title: 'Tolak Lupa Absen');
    if (note == null || !mounted) return;
    try {
      await GuruService.rejectForgotAttendance(item.id, note);
      _load(reset: true);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
